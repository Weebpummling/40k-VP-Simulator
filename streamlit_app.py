...
def compute_ev(events):
    """
    Given a list of (pts, [p1..p5]) where p# is the percentage chance (0–100),
    return a list of 5 EVs (one per round) under the independent‐events model.
    """
    evs = []
    for r in range(5):
        ev_r = sum(pts * (probs[r] / 100) for pts, probs in events)
        evs.append(ev_r)
    return evs
...
# ─────────────────────────────────────────────────────────────────────────────
# 2) Base events template (two events per card)
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# 3) Build editable DataFrame (one row per card, two events renamed)
# ─────────────────────────────────────────────────────────────────────────────

columns = [
    "Active", "Card",
    "Initial VP", *(f"Initial VP Round {i} (%)" for i in range(1,6)),
    "Additional VP", *(f"Additional VP Round {i} (%)" for i in range(1,6)),
]
...
for card, evs in base_events.items():
    ...
    row["Initial VP"] = pts1
    for i in range(5):
        row[f"Initial VP Round {i+1} (%)"] = pr1[i]
    # Additional event or blanks
    if len(evs) > 1:
        pts2, pr2 = evs[1]
    else:
        pts2, pr2 = 0, [0]*5
    ...
    for i in range(5):
        row[f"Additional VP Round {i+1} (%)"] = pr2[i]
    rows.append(row)
...
for _, r in edited[edited["Active"]].iterrows():
    ...
    pts1 = r["Initial VP"]
    if pts1 > 0:
        probs1 = [r[f"Initial VP Round {i} (%)"] for i in range(1,6)]
        evs.append((pts1, probs1))
    # Additional
    pts2 = r["Additional VP"]
    if pts2 > 0:
        probs2 = [r[f"Additional VP Round {i} (%)"] for i in range(1,6)]
