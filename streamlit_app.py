...
if save_name and st.sidebar.button("Save Profile"):
    # Gather current widget values from session_state
...
for i in range(1, 6):
    ...
    used = c3.multiselect(f"Cards used in R{i} (remove from pool)", options=list(BASE_CARDS.keys()), key=f"used_{i}")
...
# ─────────────────────────────────────────────────────────────────────────────
# will compute exp_mission after simulation
# placeholder until after simulation:
...
mapping = {
    "Guaranteed (100%)":100, "Highly Likely (80%)":80, "Likely (70%)":70,
    "Maybe (50%)":50, "Unlikely (30%)":30, "Highly Unlikely (10%)":10
}

available_cards = [c for c in BASE_CARDS if c not in removed_cards_all]
selected = st.multiselect("Select cards to include", options=available_cards, default=available_cards)
...
for card in selected:
    ...
    cols = st.columns(5)
    probs = [
        mapping[cols[j].selectbox(f"R{j+1} chance", options=categories, index=3, key=f"{card}_init_{j}")]
        for j in range(5)
    ]
    ...
    if cfg['additional']:
        ...
        cols2 = st.columns(5)
        probs2 = [
            mapping[cols2[j].selectbox(f"R{j+1} add", options=categories, index=3, key=f"{card}_add_{j}")]
            for j in range(5)
        ]
...
proj_placeholder.markdown("## 📊 Projected Victory Points")
m1, m2 = proj_placeholder.columns(2)
